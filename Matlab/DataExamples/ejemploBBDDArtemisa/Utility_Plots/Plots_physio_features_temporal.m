function Plots_physio_features_temporal(features_s,physio_sig,n_feat,feat_name,plot_units,labels_s,exclude_vec)
...
if (~ (sum(voluntaria==exclude_vec)>0))    
    if(~ (isempty(features_s.features{voluntaria,1}.EH.Video.(field_s))))
        ...
        num_windows_v1=max([num_windows , length(features_s.features{voluntaria,1}.EH.Video.(field_s)(:,n_feat))]);
        num_windows_v2=max([num_windows , length(features_s.features{voluntaria,1}.EH.Video.(field_s)(:,n_feat))]);
        num_windows_v3=max([num_windows , length(features_s.features{voluntaria,1}.EH.Video.(field_s)(:,n_feat))]);
        num_windows_v4=max([num_windows , length(features_s.features{voluntaria,1}.EH.Video.(field_s)(:,n_feat))]);

function Results_BioSpeech = EMP_DTE_Physio_Artemisa_JustTrain_par(info)
...
function test(peri,labels)
    
    
    
    for i=1+5*(labindex-1):5*(labindex)
    
%       filoeID=fopen(sprintf('log_%i_%i.txt',labindex,i),'w');
      %Display some info
      fprintf('Running %d, Trainning and Testing...\n',i);
      
      %Create name file to store data
      fname = strcat('ResultsArtemisa_test_V',num2str(i));

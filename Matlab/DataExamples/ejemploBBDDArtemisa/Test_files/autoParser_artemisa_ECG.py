...
for origFileName in file_names:
	...
	g.write("timestamp;packet_id;SKT;BVP;GSR;EMG;Resp;ECG\n")
	timestampPrev = -1
	ifError = 0
	primera_vez = 0
	for linea in f:
		...
		for group in step1[2:len(step1)]:
			...
			if len(split3)!= 8: #10 is the number of sensors
				print("error for the number of measurements in group: ")
				print(len(split3))
				print(group)
			else:
				g.write(timestamp+";"+split3[0]+";"+split3[2]+";"+split3[3]+";"+split3[4]+";"+split3[5]+";"+split3[6]+";"+split3[7]+";"+"\n")
